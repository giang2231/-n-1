...
-- Xóa dữ liệu cũ của bảng này (nếu có) để nạp mới cho đồng bộ

USE QuanLyDienThoai;
GO

-- =========================================================================
-- BƯỚC 1: QUÈT SẠCH CÁC BẢNG THEO THỨ TỰ TỪ CON ĐẾN CHA ĐỂ GIẢI PHÓNG KHÓA NGOẠI
-- =========================================================================
...
-- Xóa bảng hãng cũ để nạp lại bản full 7 cột
DELETE FROM HangCungCap;
GO

-- =========================================================================
-- BƯỚC 2: BƠM LẠI ĐẦY ĐỦ 20 HÃNG CUNG CẤP VỚI TRỌN BỘ 7 CỘT CHUẨN XÁC
-- =========================================================================
...
-- =========================================================
-- BƯỚC 2: BƠM LẠI 50 MẪU ĐIỆN THOẠI (BẢN CHUẨN ĐẦY ĐỦ 13 CỘT CỦA BẠN)
-- =========================================================
...
-- 🎮 SONY, ASUS, GOOGLE, ONEPLUS (NCC08 -> NCC11 - 9 Mẫu)
...
-- =========================================================================
-- BƯỚC 1: XÓA SẠCH DỮ LIỆU TẦNG THẤP TRƯỚC ĐỂ GIẢI PHÓNG KHÓA NGOẠI
-- =========================================================================
DELETE FROM ChiTietHoaDon;
DELETE FROM ChiTietPhieuNhap; -- PHẢI XÓA ÔNG CON ĐANG ÔM IMEI NÀY TRƯỚC!
DELETE FROM PhieuBaoTri;
GO

-- BƯỚC 2: BÂY GIỜ XÓA BẢNG PHIÊN BẢN SẼ THÀNH CÔNG 100%
DELETE FROM ChiTietDienThoai;
GO

-- =========================================================================
-- BƯỚC 3: BƠM LẠI TOÀN BỘ CÁC PHIÊN BẢN CẤU HÌNH (MÀU SẮC, GIÁ TIỀN CHUẨN)
-- =========================================================================
INSERT INTO ChiTietDienThoai (MaChiTiet, MaMay, DungLuong, MauSac, GiaNhap, GiaBan, TrangThai) VALUES
-- 🍎 CÁC PHIÊN BẢN IPHONE 15 PRO MAX (MM01)
...
-- =========================================================================
-- 2. TẠO CÁC PHIÊN BẢN CẤU HÌNH & GIÁ TIỀN (Tầng 2)
-- =========================================================================
-- Lưu ý: Trỏ về các mã máy (MM01: 15 Pro Max, MM11: S24 Ultra, MM21: Xiaomi 14 Ultra...)


-- =========================================================================
-- 3. NHẬP HÀNG VÀO KHO (SINH MÃ IMEI THỰC TẾ)
-- =========================================================================
-- Tạo 1 phiếu nhập khổng lồ do Quản lý Giang nhập từ Apple & Samsung
...
-- Bắn các mã IMEI vào kho (Khớp với phiếu nhập và mã cấu hình chi tiết ở trên)
-- Lưu ý: Cột MaChiTietSP là do lúc nãy chúng ta dùng lệnh ALTER TABLE đổi tên để trỏ đúng chuẩn 3NF
INSERT INTO ChiTietPhieuNhap (MaPhieu, MaChiTietSP, SoIMEI, DonGiaNhap, SoLuong, TrangThaiIMEI) VALUES
-- 3 con iPhone 15 Pro Max 256GB (CT01)
...
-- Thợ phụ trách chính: NV003 (Trần Kỹ Thuật), Đã xử lý xong hoàn toàn năm 2025
...
-- Các ca máy lỗi nhẹ, xử lý lấy ngay trong ngày
...
-- Loạt phiếu bảo trì từ tháng 05 -> 12/2025
...
-- Bước sang đầu năm hiện tại 2026: Trạng thái hoàn thành bàn giao
...
-- 🛠️ CÁC PHIẾU ĐẶC BIỆT: TIẾP NHẬN NHƯNG KHÔNG SỬA ĐƯỢC (Trả máy rách/hoàn tiền)
...
-- 🔔 CÁC PHIẾU Ở TRẠNG THÁI: "ĐÃ SỬA XONG" (Đang chờ khách mang biên nhận đến lấy máy)
...
-- ⏳ CÁC PHIÊN BẢN ĐANG Ở TRẠNG THÁI: "ĐANG SỬA CHỮA" (Thợ kỹ thuật đang mổ máy trên bàn)
...
-- 📥 CÁC PHIẾU NÓNG HỔI NHẤT MỚI TIẾP NHẬN: "ĐÃ TIẾP NHẬN" (Mới nhận từ quầy thu ngân chuyển vào)
...
-- 📅 NĂM 2024 (Chiếm doanh thu nền móng lịch sử)
...
-- 📅 NĂM 2025 (Năm bùng nổ doanh số đỉnh cao)
...
-- 📅 NĂM hiện tại 2026 (Rất thời sự và cập nhật)
...
-- ⏳ CÁC HÓA ĐƠN TRẠNG THÁI KHÁC (Để Demo kiểm thử tính năng nâng cao trên phần mềm)
...
-- =========================================================================
-- BƠM 50 CHI TIẾT HÓA ĐƠN (ĐÃ BỎ CỘT TỰ TĂNG ĐỂ SQL SERVER TỰ QUẢN LÝ)
-- =========================================================================
INSERT INTO ChiTietHoaDon (MaHD, SoIMEI, DonGia, SoLuong, MaChiTietSP) VALUES

-- 📅 CÁC HÓA ĐƠN NĂM 2024
...
-- ⏳ CÁC HÓA ĐƠN ĐANG CHỜ HOẶC ĐÃ HỦY
...
-- =========================================================
-- BƯỚC 2: BƠM 50 PHIẾU NHẬP HÀNG (QUY MÔ TIỀN TRĂM TRIỆU ĐẾN TỶ)
-- =========================================================
INSERT INTO PhieuNhap (MaPhieu, MaHang, MaNV, NgayNhap, TongTien, TrangThai, GhiChu) VALUES

-- 📅 NĂM 2024: Các lô hàng nền tảng
...
-- 📅 NĂM 2025: Bùng nổ nhập hàng kinh doanh
('PN16', 'NCC01', 'NV004', '2025-01-10', 1200000000, N'Đã nhập', N'Lô iPhone 15 Pro Max số lượng lớn đón Tết'),
...
-- ⏳ CÁC PHIẾU ĐANG CHỜ GIAO HOẶC LỖI (Dùng để test tính năng Lọc)
...
-- (Tùy chọn) Reset lại bộ đếm tự động tăng về 0 
DBCC CHECKIDENT ('ChiTietPhieuNhap', RESEED, 0);
GO

-- =========================================================================
-- BƠM 50 MÃ IMEI VÀO KHO HÀNG (SẴN SÀNG ĐỂ BÁN)
-- =========================================================================
INSERT INTO ChiTietPhieuNhap (MaPhieu, SoIMEI, DonGiaNhap, SoLuong, TrangThaiIMEI, MaChiTietSP) VALUES

-- 📦 Lô PN01: iPhone 15 Pro Max (CT01: 256GB - 27.5tr, CT02: 512GB - 32tr)
...
('PN01', '358111000222005', 32000000, 1, N'Đã bán', 'CT02'), -- 1 máy đã được bán ra

-- 📦 Lô PN02: Galaxy S24 Ultra (CT12: 256GB - 25tr, CT13: 512GB - 28tr)
...
-- 📦 Lô PN03: Xiaomi 14 Ultra (CT23 - 24.5tr) & Xiaomi 14 (CT25 - 17tr)
...
-- 📦 Lô PN08: Galaxy Z Fold 5 (CT17 - 31tr) & Z Flip 5 (CT19 - 16.5tr)
...
-- 📦 Lô PN14: Galaxy A55 Tầm trung (CT21 - 8tr)
...
-- 📦 Lô PN21: iPhone 15 Thường (CT08 - 17.5tr)
...
-- 📦 Lô PN10: Poco X6 Pro Gaming (CT27 - 7.4tr)
...
-- 📦 Lô PN04: Oppo Reno 11 Pro (CT31 - 13tr)
...
-- 📦 Lô PN06: Vivo X100 Pro (CT32 - 20.5tr)
...
-- 📦 Lô PN09: Asus ROG Phone 8 Pro (CT34 - 25.5tr)
...
CREATE OR ALTER PROCEDURE sp_ThemDienThoai
    ...
    @TrangThai BIT = 1,
    @DanhMuc NVARCHAR(50) = NULL -- Thêm tham số mới ở đây
AS
BEGIN
    SET NOCOUNT ON;
...
-- 2. Cập nhật Proc Sửa máy (sp_SuaDienThoai)
...
BEGIN
    ...
    SET TenMay = @TenMay, 
        ...
        TrangThai = @TrangThai,
        DanhMuc = @DanhMuc -- Cập nhật giá trị mới vào đây
    WHERE MaMay = @MaMay;
    RETURN 1;
END
GO
-- 3. Xóa Dòng Máy (Xóa mềm cả Cha lẫn Con)
CREATE OR ALTER PROCEDURE sp_XoaDienThoai
    @MaMay NVARCHAR(20)
AS
BEGIN
    -- 1. Đổi trạng thái Dòng máy thành Ngừng bán
    UPDATE DienThoai SET TrangThai = 0 WHERE MaMay = @MaMay;
    -- 2. Tự động đổi trạng thái toàn bộ Phiên bản của máy đó thành Ngừng bán
...
-- =========================================================================
-- PHẦN 2: PROCEDURE CHO BẢNG CON (PHIÊN BẢN - ChiTietDienThoai)
-- =========================================================================

-- 4. Thêm / Sửa Phiên Bản (Thuật toán UPSERT thông minh)
...
AS
BEGIN
    -- Kiểm tra xem Mã phiên bản này đã tồn tại chưa
    IF EXISTS (SELECT 1 FROM ChiTietDienThoai WHERE MaChiTiet = @MaChiTiet)
    BEGIN
        -- Nếu ĐÃ TỒN TẠI -> Thực hiện lệnh SỬA (UPDATE)
        UPDATE ChiTietDienThoai
        SET DungLuong = @DungLuong,
            MauSac = @MauSac,
            GiaNhap = @GiaNhap,
            GiaBan = @GiaBan,
            TrangThai = 1 -- Đảm bảo nó đang được bán
        WHERE MaChiTiet = @MaChiTiet;
    END
    ELSE
    BEGIN
        -- Nếu CHƯA TỒN TẠI -> Thực hiện lệnh THÊM MỚI (INSERT)
...
-- Lệnh thêm cột Danh mục sản phẩm
ALTER TABLE DienThoai ADD DanhMuc NVARCHAR(50);
GO


-- 1. THỦ TỤC QUÉT MÃ VẠCH (SQL tự kiểm tra kho và trả về thông tin)
...
BEGIN
    ...
    -- Nếu qua ải, nhả dữ liệu lên cho C# hiển thị
...
-- 2. THỦ TỤC CHỐT HÓA ĐƠN ĐẠI PHÁP (Xử lý chuỗi IMEI, tính tiền, trừ kho, sinh mã)
CREATE OR ALTER PROCEDURE sp_ChotHoaDon
    @SoDienThoai_KH NVARCHAR(20),
    @MaNV NVARCHAR(20),
    @GiamGia DECIMAL(18,2),
    @GhiChu NVARCHAR(250),
    @DanhSachIMEI NVARCHAR(MAX) -- C# sẽ gửi xuống dạng: "IMEI01,IMEI02,IMEI03"
AS
BEGIN
    BEGIN TRY
        BEGIN TRAN; -- KHÓA GIAO DỊCH: Sai 1 ly là hoàn tác toàn bộ

        -- BẮT LỖI TẬN GỐC
        IF (@DanhSachIMEI IS NULL OR @DanhSachIMEI = '')
            THROW 50002, N'LỖI: Chứng từ trống! Chưa có sản phẩm nào để xuất bán.', 1;

        -- Xác định khách hàng (Nếu sđt trống -> Khách vãng lai)
        ...
        -- Tự động sinh mã Hóa Đơn (Đỡ tốn công C# phải đẻ mã)
        DECLARE @MaHD NVARCHAR(20) = 'HD' + FORMAT(GETDATE(), 'yyMMddHHmmss');

        -- Tự động tính Tổng Tiền từ chuỗi IMEI (Dùng hàm STRING_SPLIT có sẵn của SQL)
        ...
        -- 2. Lưu Chi Tiết Hóa Đơn (Tự bóc tách chuỗi ra từng dòng Insert)
        ...
        COMMIT TRAN; -- Chốt sổ thành công
    END TRY
    BEGIN CATCH
        ROLLBACK TRAN; -- Hủy bỏ toàn bộ thao tác nếu vướng 1 lỗi nhỏ
        THROW; -- Quăng câu chửi lên C#
    END CATCH
END
GO
 ...
 ---hãng cung cấp
 -- =================================================================================
-- 1. STORED PROCEDURE: THÊM MỚI HÃNG Cung Cấp (INSERT)
-- =================================================================================
...
BEGIN
    ...
    BEGIN TRY
        -- Kiểm tra xem Mã Hãng hoặc Tên Hãng đã tồn tại chưa
        ...
        -- Thực hiện chèn dữ liệu nếu không có lỗi trùng lặp
    ...
    END TRY
    BEGIN CATCH
        -- Bắt và hiển thị lỗi hệ thống nếu có
...
-- =================================================================================
-- 2. STORED PROCEDURE: CẬP NHẬT HÃNG SẢN XUẤT (UPDATE)
-- =================================================================================
...
BEGIN
    ...
    BEGIN TRY
        -- Kiểm tra xem hãng cần sửa có tồn tại không
        ...
        -- Kiểm tra trùng tên với hãng khác khi đổi tên hãng
...
-- =================================================================================
-- 3. STORED PROCEDURE: XÓA HÃNG SẢN XUẤT (SOFT DELETE - CHUYỂN TRẠNG THÁI)
-- =================================================================================
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Thực hiện xóa mềm: Cập nhật TrangThai = 0 (Ngừng hoạt động / Đã xóa)
        UPDATE HangCungCap
        SET TrangThai = 0
        WHERE MaHang = @MaHang;
...
--khách hàng
-- =================================================================================
-- 1. STORED PROCEDURE: THÊM MỚI KHÁCH HÀNG (INSERT)
-- =================================================================================
CREATE or alter PROCEDURE sp_InsertKhachHang
  @MaKH VARCHAR(20),
    ...
    @TongChiTieu DECIMAL(18,2) = 0,    -- ✅ THÊM "= 0" VÀO ĐÂY (Thành tham số không bắt buộc)
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Kiểm tra trùng Số điện thoại (Tránh tạo tài khoản ảo trùng SĐT)
        ...
        -- Nếu hệ thống không truyền Ngày tạo, tự động lấy ngày giờ hiện tại
       IF @NgayTao IS NULL
            SET @NgayTao = GETDATE();
...
-- =================================================================================
-- 2. STORED PROCEDURE: CẬP NHẬT THÔNG TIN KHÁCH HÀNG (UPDATE)
-- =================================================================================
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Kiểm tra trùng SĐT với người khác khi thay đổi số điện thoại
        ...
        -- Thực hiện cập nhật các trường (Giữ nguyên NgayTao)
        UPDATE KhachHang
        SET HoTen = @HoTen,
            GioiTinh = @GioiTinh,
            NgaySinh = @NgaySinh,
...
-- =================================================================================
-- 3. STORED PROCEDURE: XÓA KHÁCH HÀNG (DELETE CÓ KIỂM TRA KHÓA NGOẠI)
-- =================================================================================
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Thực hiện xóa vật lý khỏi bảng
        DELETE FROM KhachHang WHERE MaKH = @MaKH;

        PRINT N'Xóa thông tin khách hàng thành công!';
    END TRY
    BEGIN CATCH
        -- Bắt lỗi vi phạm ràng buộc khóa ngoại (Lỗi số 547 trong SQL Server)
        -- xảy ra nếu khách hàng này đã có hóa đơn bán hàng hoặc phiếu bảo trì trong hệ thống
        IF ERROR_NUMBER() = 547
        BEGIN
...
GO
--hóa đơn
-- 1. CẬP NHẬT LẠI PROCEDURE LẤY DANH SÁCH HÓA ĐƠN
...
BEGIN
    SELECT hd.MaHD AS [Mã HĐ], hd.NgayLap AS [Ngày Lập], 
           ...
           -- SỬA TẠI ĐÂY: So sánh với chuỗi chữ thay vì số 1 để tránh lỗi ép kiểu
...
-- 2. CẬP NHẬT LẠI PROCEDURE HỦY HÓA ĐƠN
...
BEGIN
    BEGIN TRY
        ...
        -- SỬA TẠI ĐÂY: So sánh với chuỗi chữ N'Đã Hủy'
        ...
        -- SỬA TẠI ĐÂY: Cập nhật thành chuỗi chữ N'Đã Hủy' thay vì số 0
...
END
GO 
-- SỬA LẠI PROCEDURE CẬP NHẬT HÓA ĐƠN
CREATE OR ALTER PROCEDURE sp_UpdateHoaDon
    ...
    @GiamGia DECIMAL(18,2),
    @TrangThai NVARCHAR(50), -- Đã sửa thành NVARCHAR
    @GhiChu NVARCHAR(500)
AS
BEGIN
    SET NOCOUNT ON;
...
-- =================================================================================
-- 1. STORED PROCEDURE: THÊM CHI TIẾT HÓA ĐƠN (TỰ ĐỘNG CHUYỂN TRẠNG THÁI IMEI)
-- =================================================================================
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 2. Đảm bảo số lượng mặc định là 1 nếu bán theo số IMEI độc nhất
        IF @SoLuong IS NULL OR @SoLuong <= 0 
            SET @SoLuong = 1;

        -- 3. Chèn dữ liệu vào bảng ChiTietHoaDon
        INSERT INTO ChiTietHoaDon (MaHD, SoIMEI, DonGia, SoLuong, MaChiTietSP)
        VALUES (@MaHD, @SoIMEI, @DonGia, @SoLuong, @MaChiTietSP);

        -- 4. TỰ ĐỘNG CẬP NHẬT TRẠNG THÁI MÁY TRONG KHO THÀNH 'ĐÃ BÁN'
        -- (Đồng bộ logic nghiệp vụ chốt hàng trực tiếp dưới database)
        UPDATE ChiTietPhieuNhap 
        SET TrangThaiIMEI = N'Đã bán' 
        WHERE SoIMEI = @SoIMEI;
...
-- =================================================================================
-- 2. STORED PROCEDURE: CẬP NHẬT CHI TIẾT HÓA ĐƠN (CẬP NHẬT GIÁ BÁN/MÃ CHI TIẾT)
-- =================================================================================
...
BEGIN
    ...
    BEGIN TRY
        -- Kiểm tra dòng chi tiết hóa đơn cần sửa có tồn tại không
        ...
        -- Thực hiện cập nhật thông tin đơn giá hoặc mã liên kết sản phẩm
...
-- =================================================================================
-- 3. STORED PROCEDURE: XÓA 1 DÒNG CHI TIẾT (TỰ ĐỘNG HOÀN TRẢ TRẠNG THÁI IMEI VỀ KHO)
-- =================================================================================
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 1. Xóa dòng chi tiết khỏi hóa đơn
        DELETE FROM ChiTietHoaDon WHERE MaHD = @MaHD AND SoIMEI = @SoIMEI;

        -- 2. Tự động hoàn trả trạng thái thiết bị này về trạng thái 'Trong kho' để có thể bán tiếp
        UPDATE ChiTietPhieuNhap 
        SET TrangThaiIMEI = N'Trong kho' 
        WHERE SoIMEI = @SoIMEI;
...
-- =================================================================================
-- 1. STORED PROCEDURE: TẠO PHIẾU NHẬP HÀNG MỚI (INSERT)
-- =================================================================================
...
BEGIN
    ...
    END TRY
    BEGIN CATCH
        THROW; -- Ném lỗi thẳng lên WinForms để C# bắt bằng catch (Exception ex)
    END CATCH
END
GO

-- =================================================================================
-- 2. STORED PROCEDURE: CẬP NHẬT PHIẾU NHẬP HÀNG (UPDATE)
-- (Lưu ý: Không cho phép cập nhật Ngày nhập để giữ tính toàn vẹn lịch sử)
-- =================================================================================
...
-- =================================================================================
-- 3. STORED PROCEDURE: HỦY PHIẾU NHẬP HÀNG (TRANSACTION BẢO VỆ KHO)
-- =================================================================================
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 2. Kiểm tra xem phiếu đã bị hủy trước đó chưa
        ...
        -- 3. THỰC HIỆN XÓA MỀM: Chuyển trạng thái về "Đã hủy"
        UPDATE PhieuNhapHang
        SET TrangThai = N'Đã hủy',
            GhiChu = ISNULL(GhiChu, '') + N' [Hệ thống ghi nhận: Phiếu nhập đã bị hủy]'
        WHERE MaPhieu = @MaPhieu;

        -- 4. LOGIC ĐẢO NGƯỢC KHO (Tương tự như lúc Hủy Hóa Đơn)
        -- Khi hủy phiếu nhập, toàn bộ máy (IMEI) thuộc phiếu này không được phép bán nữa
        UPDATE ChiTietPhieuNhap
...
-- =================================================================================
-- 1. STORED PROCEDURE: THÊM MỚI NHÂN VIÊN (INSERT)
-- =================================================================================
CREATE PROCEDURE sp_InsertNhanVien
    ...
    @CMND VARCHAR(20),
    @SoDienThoai VARCHAR(15),
    @Email VARCHAR(100) = NULL,          -- Cho phép Email trống nếu nhân viên không có
    @DiaChi NVARCHAR(200),
    @ChucVu NVARCHAR(50),
    @LuongCoBan DECIMAL(18,2) = 0,       -- Mặc định lương 0 nếu chưa nhập
    @TenDangNhap VARCHAR(50),
    @MatKhau VARCHAR(255),
   @TrangThai BIT = 1 -- Mặc định tạo mới là đang làm việc
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        -- 2. Kiểm tra trùng Tên Đăng Nhập (Cực kỳ quan trọng)
...
-- =================================================================================
-- 2. STORED PROCEDURE: CẬP NHẬT THÔNG TIN NHÂN VIÊN (UPDATE)
-- =================================================================================
CREATE PROCEDURE sp_UpdateNhanVien
    ...
    @TenDangNhap VARCHAR(50),
   @TrangThai BIT
    -- KHÔNG CẬP NHẬT MẬT KHẨU Ở ĐÂY (Mật khẩu nên làm 1 hàm đổi mật khẩu riêng cho bảo mật)
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        -- Kiểm tra trùng CMND với người khác
...
-- =================================================================================
-- 3. STORED PROCEDURE: XÓA NHÂN VIÊN (DELETE)
-- =================================================================================
...
BEGIN
    ...
    END TRY
    BEGIN CATCH
        -- Bắt lỗi khóa ngoại (Lỗi 547)
        -- Nếu nhân viên đã từng lập Hóa Đơn hoặc Nhập Hàng thì không thể xóa
        IF ERROR_NUMBER() = 547
        BEGIN
